...
describe('Drivers daos', () => {
    ...
    describe('addCab', () => {
        let spy;
        it('should add a new cab on request', async () => {
            await resetAndMockDB(db => {
                db.cabs.create = async value => ({
                    get: () => value
                });
                spy = jest.spyOn(db.cabs, 'create');
            });
            const { addCab } = require('daos/cabsDao');
            const { driverId, cabModel, cabType, cabLicense } = mockCabs;

            await addCab(driverId, cabModel, cabType, cabLicense);
