...
describe('/driver route tests ', () => {
    ...
    it('should return 404', async () => {
        ...
        const res = await server.inject({
            method: 'GET',
            url: '/bookings/driver/3'
        });

        expect(res.statusCode).toEqual(404);
        expect(res.result.message).toEqual('No bookings found for driverId 3');
    });

    it.only('should return all the bookings ', async () => {
        server = await resetAndMockDB(async allDbs => {
            allDbs.bookings.$queryInterface.$useHandler(function (query) {
                if (query === 'findById') {
                    return bookings;
        ...
        expect(firstBooking.id).toEqual(bookings.driverId);
